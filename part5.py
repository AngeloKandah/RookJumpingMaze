...
from part2 import traverseBoard, evalNumber, printevalBoard

def hillDescent(iterations, board, p):
    ...
    for i in range(iterations):
        ...
        if(new <= besteval or p > random.random()):
            besteval = new
...
def checkMove(move, evalboard):
    ...
    if(col > evalboard.shape[0]/2):
        ...
        if(evalboard[row,col] + col + 1 > board.shape[0]):
            num = random.randint(1, round(evalboard.shape[0]/2)+1)       
...
if __name__ == "__main__":
    ...
    p = float(input("Uphill step probability? "))
    while(p > 1 or p < 0):
        p = float(input("Uphill step probability? "))
    board = makeBoard(5)
    hillboard = hillDescent(x, board, p)
